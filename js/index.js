...
// focus
const colors = ['#A0C5CD', '#FECC4B', '#F8D8BE', '#D64045'];
links.forEach(link => {
  link.addEventListener('focus', () => {
    const choice = Math.floor(Math.random() * colors.length);
    link.style.color = colors[choice];
  });
});
...
window.addEventListener('resize', (event) => {
  ...
  document.body.style.backgroundColor = innerWidth > 1000 ? colors[0]
    : innerWidth > 800 ? colors[1]
      : innerWidth > 600 ? colors[2] : colors[3];
});
